...
public class NVProcessor implements Runnable{
	...
	public NVProcessor(int queueGroup,int processor_Sleeptime,int channelId,int tenantGroupId,TenantGroupObj tenantGrpObj){
		...
		_logger 				= new LogHelper(NVProcessor.class).getLogger(LogModuleFactory.getModule("NVProcessor"), String.valueOf(queueGroup)+"_"+String.valueOf(channelId));		
	        ...
	        try{
	        	if(dbConnection!=null){
	        		...
	        		String selectFromRaw="SELECT sum(D020) as agg_D010 ,sum(D040) as agg_D040,sum(D030) as agg_D030,sum(D060) as agg_D060,sum(D070) as agg_D070,sum(D090) as agg_D090,sum(D100) as agg_D100,sum(D130) as agg_D130,max(D051) as agg_D051 FROM APP.QSTATS_RAW_DATA WHERE channel = ? AND tenantgroup_pkey = ? ";
	        		 if (_debugLog) { 
	        				_logger.debug("NVProcessor:: in updateAggregateTable channelId and tenant group id passed : "+channelId+" & "+tgId );
	        			}
					...
					if(selectResultSet!=null){
						...
						if (_debugLog) { 
							_logger.debug("updateAggregateTable:: Data Aggregated from RAW - "+selectFromRaw+",Aggregated Data: " +
									"agg_D020 Call handled since morning ["+agg_D020+"] " +											
									"agg_D030 Call Abandoned since morning ["+agg_D030+"] " +
									"agg_D040 Number of Contacts waiting ["+agg_D040+"] " +
									"agg_D060 Call back PENDING immediate ["+agg_D060+"] " +
									"agg_D070 Call back PENDING scheduled ["+agg_D070+"] " +
									"agg_D090 Call back ACCEPTED ["+agg_D090+"] " +
									"agg_D130 Total calls at Agent ["+agg_D130+"] " +
									"agg_D051 Duration for Longest call waiting ["+agg_D051+"] ");
						}
					}
					else{
						if (_errorLog) {
							_logger.error("updateAggregateTable :: ResultSet is Empty.");
						}
					}
					...
					if (_errorLog) {
						_logger.error("updateAggregateTable :: Error fetching db connection ");
					}
				}
	        }catch(Exception e){
					if (_errorLog) {
						_logger.error("updateAggregateTable :: Error in Executing Query--- ", e);
					}
				}
				...
				if (_errorLog) {
					_logger.error("updateAggregateTable ::  Error  "
							+  " table.", e);
				}
			} finally {
				...
				if (dbConnection != null) {
					try {
						...
						if (_errorLog) {
							_logger.error("updateAggregateTable :: Error relasing derby connection", ex);
						}
					}
					//dbConnection = null;
				}
	...
	private void updateDisplayTable(int queueGroup) {
		...
		try {
	        ...
	        for(String qKey:queryList.keySet()){	         
	        	...
	        	if (_debugLog) {
					_logger.debug("updateDisplayTable:: Query obtained from the ProcessorQueries.prop file for QueueGroup----"+ queueGroup);
				}
				if (dbConnection != null) {					
					...
					if(rs != null){						
					    ...
					    finalString.append("UPDATE APP.QSTATS_DISPLAY_DATA SET ").append(columnNames).append(" WHERE PKEY=").append(rs.getString("PKEY"));
