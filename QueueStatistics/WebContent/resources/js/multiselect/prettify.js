...
(function() {
	...
	var C_KEYWORDS = FLOW_CONTROL_KEYWORDS
			+ "auto case char const default "
			+ "double enum extern float goto int long register short signed sizeof "
			+ "static struct switch typedef union unsigned void volatile ";
	var COMMON_KEYWORDS = C_KEYWORDS
			+ "catch class delete false import "
			+ "new operator private protected public this throw true try typeof ";
	var CPP_KEYWORDS = COMMON_KEYWORDS
	...
	/**
	 * A class that indicates a section of markup that is not code, e.g. to
	 * allow embedding of line numbers within code listings.
	 */
	 ...
	 * "in" keyword since it's not a keyword in many languages, and might be
	 * used as a count of inches.
	 * 
	 * <p>
	...
	var REGEXP_PRECEDER_PATTERN = function() {
		var preceders = [ "!", "!=", "!==", "#", "%", "%=", "&", "&&", "&&=",
				...
				">>", ">>=", ">>>", ">>>=", "?", "@", "[", "^", "^=", "^^",
				"^^=", "{", "|", "|=", "||", "||=",
				"~" /* handles =~ and !~ */, "break", "case", "continue",
	...
	function attribToHtml(str) {
		return str.replace(pr_amp, '&amp;').replace(pr_lt, '&lt;').replace(
				pr_gt, '&gt;').replace(pr_quot, '&quot;');
	}
	...
	function htmlToText(html) {
		...
		// These should be rare since most browsers convert them to normal
		// chars.
			...
			if (end >= 0) {
				...
				if (!isNaN(codePoint)) {
					html = (html.substring(0, pos)
							+ String.fromCharCode(codePoint) + html
							.substring(end + 1));
				}
			}
		}
	...
	function combinePrefixPatterns(regexs) {
		...
		function caseFoldCharset(charSet) {
			...
			for (var i = inverse ? 1 : 0, n = charsetParts.length; i < n; ++i) {
				...
				if (!(end < 65 || start > 122)) {
					if (!(end < 65 || start > 90)) {
						ranges.push([ Math.max(65, start) | 32,
								Math.min(end, 90) | 32 ]);
					}
					if (!(end < 97 || start > 122)) {
						ranges.push([ Math.max(97, start) & ~32,
								Math.min(end, 122) & ~32 ]);
					}
				}
			}
		...
		function allowAnywhereFoldCaseAndRenumberGroups(regex) {
			...
			// Walk over and identify back references to build the
			// capturedGroups
			// mapping.
			...
			// Renumber groups and reduce capturing groups to non-capturing
			// groups
			// where possible.
	...
	function getInnerHtml(node) {
		...
		// an html description of well formed XML and the containing tag is a
		// PRE
		...
		return function(plainText) {
			...
			for (var i = 0, n = plainText.length; i < n; ++i) {
				...
				case '\t':
					...
					// nSpaces is the amount of padding -- the number of spaces
					// needed
					// to move us to the next column, where columns occur at
					// factors of
					// tabWidth.
	...
	var pr_chunkPattern = new RegExp('[^<]+' // A run of characters other
												// than '<'
			+ '|<\!--[\\s\\S]*?--\>' // an HTML comment
	 ...
	 * {@link #PR_PLAIN}), converting tags which are significant for
	 * tokenization (<br>) into their textual equivalent.
	 * 
	 * @param {string}
	...
	function extractTags(s) {
		// since the pattern has the 'g' modifier and defines no capturing
		// groups,
		// this will return a list of all chunks which we then classify and wrap
		// as
		// PR_Tokens
			...
			for (var i = 0, n = matches.length; i < n; ++i) {
				...
				if (match.length > 1 && match.charAt(0) === '<') {
					...
					if (pr_cdataPrefix.test(match)) {
						// strip CDATA prefix and suffix. Don't unescape since
						// it's CDATA
						sourceBuf.push(match.substring(9, match.length - 3));
						sourceBufLen += match.length - 12;
					} else if (pr_brPrefix.test(match)) {
						// <br> tags are lexically significant so convert them
						// to text.
						...
						if (match.indexOf(PR_NOCODE) >= 0 && isNoCodeTag(match)) {
							// A <span class="nocode"> will start a section that
							// should be
							// ignored. Continue walking the list until we see a
							// matching end
							// tag.
							...
							if (j < n) {
								extractedTags.push(sourceBufLen, matches.slice(
										i, j + 1).join(''));
								i = j;
	...
	function createSimpleLexer(shortcutStylePatterns, fallthroughStylePatterns) {
		...
		var tokenizer;
		(function() {
			var allPatterns = shortcutStylePatterns
		...
		/**
		 * Lexes job.source and produces an output array job.decorations of
		 * style classes preceded by the position at which they start in
		 * job.source in order.
		 * 
		 * @param {Object}
		 *            job an object like {@code source: {string} sourceText
		 *            plain text, basePos: {int} position of job.source in the
		 *            larger chunk of sourceCode. }
		 */
		var decorate = function(job) {
			var sourceCode = job.source, basePos = job.basePos;
			/**
			 * Even entries are positions in source in ascending order. Odd
			 * enties are style markers (e.g., PR_COMMENT) that run from that
			...
			for (var ti = 0, nTokens = tokens.length; ti < nTokens; ++ti) {
				...
				if (!isEmbedded) {
				...
				} else { // Treat group 1 as an embedded block of source
							// code.
					var embeddedSource = match[1];
					var embeddedSourceStart = token.indexOf(embeddedSource);
					var embeddedSourceEnd = embeddedSourceStart
							+ embeddedSource.length;
					if (match[2]) {
						// If embeddedSource can be blank, then it would match
						// at the
						// beginning which would cause us to infinitely recurse
						// on the
						// entire token, so we catch the right context in
						// match[2].
						embeddedSourceEnd = token.length - match[2].length;
						embeddedSourceStart = embeddedSourceEnd
								- embeddedSource.length;
					}
					...
					appendDecorations(basePos + tokenStart
							+ embeddedSourceStart, embeddedSource,
							langHandlerForExtension(lang, embeddedSource),
							decorations);
					// Decorate the right of the embedded section
					appendDecorations(basePos + tokenStart + embeddedSourceEnd,
							token.substring(embeddedSourceEnd), decorate,
							decorations);
				}
			}
	 ...
	 * escape. It does not recognize perl's qq() style strings. It has no
	 * special handling for double delimiter escapes as in basic, or the tripled
	 * delimiters used in python, but should work on those regardless although
	 * in those cases a single string literal may be broken up into multiple
	 ...
	 * @return {function (Object)} a function that examines the source code in
	 *         the input job and builds the decoration list.
	 */
	function sourceDecorator(options) {
		...
		if (options['verbatimStrings']) {
			// verbatim-string-literal production from the C# grammar. See issue
			// 93.
		...
		if (options['regexLiterals']) {
			...
			fallthroughStylePatterns.push([
					'lang-regex',
					new RegExp('^' + REGEXP_PRECEDER_PATTERN + '('
							+ REGEX_LITERAL + ')') ]);
		}
		...
		if (keywords.length) {
			fallthroughStylePatterns
					.push([
							PR_KEYWORD,
							new RegExp('^(?:' + keywords.replace(/\s+/g, '|')
									+ ')\\b'), null ]);
		}
		...
		[ PR_LITERAL, /^@[a-z_$][a-z_$@0-9]*/i, null ], [ PR_TYPE,
				...
				/^[a-z_$][a-z_$@0-9]*/i, null ], [ PR_LITERAL,
				new RegExp('^(?:'
				// A hex number
				+ '0x[a-f0-9]+'
		...
		return createSimpleLexer(shortcutStylePatterns,
				fallthroughStylePatterns);
	}
	 ...
	 * 
	 * @param {Object}
	 *            job like { source: {string} source as plain text,
	 *            extractedTags: {Array.<number|string>} extractedTags chunks
	 *            of raw html preceded by their position in {@code job.source}
	 *            in order decorations: {Array.<number|string} an array of
	 *            style classes preceded by the position at which they start in
	 *            job.source in order }
	 * @private
	 */
	function recombineTagsAndDecorations(job) {
		...
		var newlineRe = /\r\n?|\n/g;
		var trailingSpaceRe = /[ \r\n]$/;
		var lastWasSpace = true; // the last text chunk emitted ended with a
									// space.

		// A helper function that is responsible for opening sections of
		// decoration
		...
		function emitTextUpTo(sourceIdx) {
			if (sourceIdx > outputIdx) {
				...
				// This interacts badly with some wikis which introduces
				// paragraph tags
				// into pre blocks for some strange reason.
				// It's necessary for IE though which seems to lose the
				// preformattedness
				// of <pre> tags when their innerHTML is assigned.
				// http://stud3.tuwien.ac.at/~e0226430/innerHtmlQuirk.html
				// and it serves to undo the conversion of <br>s to newlines
				// done in
				// chunkify.
				var htmlChunk = textToHtml(
						tabExpander(sourceText.substring(outputIdx, sourceIdx)))
						.replace(
								lastWasSpace ? startOrSpaceRe : adjacentSpaceRe,
								'$1&nbsp;');
				// Keep track of whether we need to escape space at the
				// beginning of the
				// next chunk.
				lastWasSpace = trailingSpaceRe.test(htmlChunk);
				// IE collapses multiple adjacient <br>s into 1 line break.
				// Prefix every <br> with '&nbsp;' can prevent such IE's
				// behavior.
		...
		while (true) {
			// Determine if we're going to consume a tag this time around.
			// Otherwise
			// we consume a decoration or exit.
			var outputTag;
			if (tagPos < extractedTags.length) {
				if (decPos < decorations.length) {
					// Pick one giving preference to extractedTags since we
					// shouldn't open
					// a new style that we're going to have to immediately close
					// in order
					// to output a tag.
	 ...
	 *            {@code { source: {string} as plain text. decorations: {Array.<number|string>}
	 *            an array of style classes preceded by the position at which
	 *            they start in job.source in order. The language handler should
	 *            assigned this field. basePos: {int} the position of source in
	 *            the larger source chunk. All positions in the output
	 *            decorations array are relative to the larger source chunk. } }
	 * @param {Array.
	...
	function langHandlerForExtension(extension, source) {
		if (!(extension && langHandlerRegistry.hasOwnProperty(extension))) {
			// Treat it as markup if the first non whitespace character is a <
			// and
			// the last non-whitespace character is a >.
			extension = /^\s*</.test(source) ? 'default-markup'
					: 'default-code';
		}
	...
	registerLangHandler(createSimpleLexer([], [ [ PR_PLAIN, /^[^<?]+/ ],
			...
			[ 'lang-in.tag', /^(<\/?[a-z][^<>]*>)/i ] ]), [ 'default-markup',
	...
	function applyDecorator(job) {
		...
		try {
			...
			/**
			 * Even entries are positions in source in ascending order. Odd
			 * entries are tags that were extracted at that position.
			 * 
			 * @type {Array.<number|string>}
			 */
			...
			// Integrate the decorations and tags back into the source code to
			// produce
	...
	function prettyPrint(opt_whenDone) {
		...
		// The loop is broken into a series of continuations to make sure that
		// we
		...
		function doWork() {
			...
			for (; k < elements.length && clock.now() < endTime; k++) {
				...
				if (cs.className && cs.className.indexOf('prettyprint') >= 0) {
					...
					// the language extension "cpp" is used to find a language
					// handler as
					...
					// make sure this is not nested in an already prettified
					// element
					var nested = false;
					...
					if (!nested) {
						// fetch the content as a snippet of properly escaped
						// HTML.
		...
		function replaceWithPrettyPrintedHtml() {
			...
			if (!isRawContent(cs)) {
				...
				// we need to change the tag to a <pre> since <xmp>s do not
				// allow
			...
			if (isIE678 && cs.tagName === 'PRE') {
				...
				for (var j = lineBreaks.length; --j >= 0;) {
					...
					lineBreak.parentNode.replaceChild(document
							.createTextNode(ieNewline), lineBreak);
				}
			}
		}
